...
- [原型](#原型)
    ...
    - [原型文档](#原型文档)
    - [口头操作说明](#口头操作说明)
- [产品使用关键AI或机器学习之API的输出入展示](#产品使用关键AI或机器学习之API的输出入展示)
    - [使用水平](#使用水平)
        - [百度人脸检测API](#百度人脸检测API)
        - [百度人像分割API](#百度人像分割API)
        - [百度拉伸图像恢复API](#百度拉伸图像恢复API)
     - [使用比较分析 ](#使用比较分析 )
        - [主要竞争者-阿里云智能人像分割（抠图）API：](#主要竞争者-阿里云智能人像分割（抠图）API：)
        - [阿里云代码测试](#阿里云代码测试)
        - [选择百度云的原因](#选择百度云的原因)
        - [价格（性价比）对比](#价格（性价比）对比)
        - [主要竞争者-FACE++人脸检测API](#主要竞争者-FACE++人脸检测API)
        - [Face++代码测试](#Face++代码测试)
        - [选择百度云的原因](#选择百度云的原因)
        - [价格（性价比）对比](#价格（性价比）对比)
     - [使用后风险报告](#使用后风险报告)
        - [百度人像分割API输出限制及其他问题](#百度人像分割API输出限制及其他问题)
        - [百度人脸检测API输出限制及其他问题](#百度人脸检测API输出限制及其他问题)
        - [百度拉伸图像恢复API输出限制及其他问题](#百度拉伸图像恢复API输出限制及其他问题)
        - [使用情况](#使用情况)
      - [加分项](#加分项)
- [迭代](#迭代)
    - [一句话版本](#一句话版本)
...
## 迭代
### 一句话版本
帮助用户快速的在线制作一张符合规范的证件照图片。

### 一分钟版本
- 本产品是为了帮助用户在线上快速制作符合规范的证件照而产生的，产品使用到3个api来帮助制作更加正规清晰的证件照图片，首先运用了百度人脸检测API，使用其中的姿态角度指标，当拍摄的人脸的pitch值和roll值越接近0，则说明人脸朝向越正，能更好的帮助用户检测脸的朝向角度，精准的角度计算设置比职业摄像师还要精确，
然后运用了百度人像分割API，对于输入的一张图片（可正常解码，且长宽比适宜），识别人体的轮廓范围，与背景进行分离，生成一种只有人像的图片，变成透明背景是制作在线证件照必不可少的一步，接下来用户只需要选择底部颜色一张证件照图片就基本完成了。本产品还加了一个另外的功能，就是使用到百度的拉伸图像恢复API，该功能属于新的api产品，目前暂时只有百度智能平台有，它可以自动识别过度拉伸的图像，将图像内容恢复成正常比例。也是辅助制作更符合规范的证件照的重要内容。
### 400秒版本
- [20*20](https://github.com/Hinata013/api/blob/master/20x20.pptx)
